const Gallery = {
    render(data, container) {
        ...
        // Mapeo dinámico y exacto basado en el Formulario real
        ...
        const html = `
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 animate-fade-in" style="animation-delay: 0.1s">
                ${data.map((row, idx) => {
                    ...
                    // Si el logo es un link de Drive, a veces no carga directo como <img>, lo intentamos
                    let hasLogo = false;
                    if (logoUrl && typeof logoUrl === 'string' && logoUrl.startsWith('http')) {
                        hasLogo = true;
                        // Fix for google drive direct image links
                    ...
                    <div class="glass-panel overflow-hidden group cursor-pointer hover:shadow-2xl hover:shadow-primary-500/10 transition-all duration-300 hover:-translate-y-2 flex flex-col h-full border border-gray-100 dark:border-gray-800 !rounded-3xl" onclick="window.dispatchEvent(new CustomEvent('rowClicked', { detail: window._galleryData[${idx}] }))">
                        ...
                        <div class="h-48 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 flex items-center justify-center relative overflow-hidden border-b border-gray-100 dark:border-gray-800/50">
                            ...
                            <div class="absolute inset-0 bg-black/0 group-hover:bg-primary-900/10 transition-colors duration-300"></div>
                        </div>
                        
                        <div class="p-6 flex-1 flex flex-col bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
                            ${cat ? `<span class="inline-block px-3 py-1.5 text-[10px] font-black tracking-widest uppercase bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-400 rounded-full mb-4 self-start">${cat}</span>` : ''}
                            
                            <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2 leading-tight group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">${name}</h3>
                            ...
                            <div class="pt-4 flex justify-between items-center mt-auto">
                                <span class="text-sm font-bold text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors">Conocer más</span>
                                <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center group-hover:bg-primary-600 group-hover:text-white transition-all shadow-sm group-hover:shadow-md group-hover:shadow-primary-500/30">
